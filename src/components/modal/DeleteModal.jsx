...
function DeleteModal({ onCloseClick, onDeleteClick, selectedData }) {
  ...
  return (
    <>
      <dialog className={cn('top-1/2 z-10 flex h-fit w-[340px] -translate-y-1/2 flex-col items-center justify-between gap-6 rounded-xl bg-grey-100 p-10')}>
        ...
        <footer className={cn('flex items-center justify-center gap-2 font-medium')}>
          ...
          <button
            ...
            className={cn('rounded bg-red-600 p-[10px] text-grey-200 hover:bg-red-200 hover:text-red-500')}
          >
            {t('knowledge.confirm')}
          </button>
        </footer>
